...
print("Logistic回归模型在测试集上的得分:", model.score(X_test, y_test))
...
model.fit(X_train, y_train)
print("装袋法分类算法在测试集上的得分:", model.score(X_test, y_test))

# 14.3.4 随机森林分类算法
model = RandomForestClassifier(n_estimators=300, max_features='sqrt', random_state=10)
model.fit(X_train, y_train)
print("随机森林分类算法在测试集上的得分:", model.score(X_test, y_test))
...
print("预测准确率随最大特征变量数变化情况：", scores)
...
print("Kappa得分:", cohen_kappa_score(y_test, pred))
...
model.fit(X_train, y_train)
print("线性回归模型在测试集上的得分:", model.score(X_test, y_test))
...
PartialDependenceDisplay.from_estimator(model, X_train, ['stop10', 'stop1'], kind='average')
PartialDependenceDisplay.from_estimator(model, X_train, ['stop10', 'stop1'], kind='individual')
PartialDependenceDisplay.from_estimator(model, X_train, ['stop10', 'stop1'], kind='both')
