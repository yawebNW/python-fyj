...
X = data.iloc[:,11:22]#设置分析所需要的特征变量
# 打印数据框架的基本信息
print(X.info())
...
#11.2.4  特征变量相关性分析
print('特征变量的相关性：\n',round(X_s.corr(method='pearson'),2))
plt.subplot(1,1,1)
...
print("特征值：\n", round(pd.DataFrame(model.explained_variance_).T,3))  # 打印特征值
print("方差贡献率：\n", round(pd.DataFrame(model.explained_variance_ratio_).T,3))  # 打印方差贡献率
...
print(model.components_)#计算主成分载荷，观察每个变量对于主成分的影响
