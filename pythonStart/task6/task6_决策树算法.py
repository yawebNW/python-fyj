...
print("预测结果前5个样本:\n", pred[:5])  # 输出预测结果
print(confusion_matrix(y_test, pred))  # 输出混淆矩阵
print(classification_report(y_test, pred))  # 输出分类报告
print("Kappa得分:", cohen_kappa_score(y_test, pred))  # 输出Kappa得分

# 二、当分裂准则为基尼指数时
model = DecisionTreeClassifier(criterion='gini', max_depth=2, random_state=10)  # 采用基尼系数作为分裂准则
model.fit(X_train, y_train)
print("模型得分（基尼指数）:", model.score(X_test, y_test))  # 输出模型在测试集上的得分
plot_tree(model, feature_names=X.columns, node_ids=True, impurity=True, proportion=True, rounded=True, precision=3)
plt.savefig('out2.pdf')  # 保存树的图像
plt.show()  # 显示树的图像
...
print("最后一棵决策时的节点数为: {} ;其alpha值为: {}".format(
    models[-1].tree_.node_count, path.ccp_alphas[-1]))  # 输出最后一个alpha值对应的节点数
...
ax.plot(path.ccp_alphas, train_scores, marker='o', label="训练样本", drawstyle="steps-post")
ax.plot(path.ccp_alphas, test_scores, marker='o', label="测试样本", drawstyle="steps-post")
ax.legend()
plt.show()  # 显示图形
...
print("最优预测准确率：", model.score(X_test, y_test))  # 输出最优模型的预测准确率
plot_tree(model, feature_names=X.columns, node_ids=True, impurity=True, proportion=True, rounded=True, precision=3)
plt.savefig('out3.pdf')  # 保存树的图像
plt.show()  # 显示树的图像
...
model.fit(X_train, y_train)
print("拟合优度：", model.score(X_test, y_test))  # 输出模型在测试集上的得分
plot_tree(model, feature_names=X.columns, node_ids=True, rounded=True, precision=3)
plt.savefig('out4.pdf')  # 保存树的图像
plt.show()  # 显示树的图像
print("文本格式的决策树：", export_text(model, feature_names=list(X.columns)))  # 输出文本格式的决策树
...
print("最优alpha值：", model.best_params_)  # 输出最优alpha值
model = model.best_estimator_
print("最优拟合优度：", model.score(X_test, y_test))  # 输出最优模型的拟合优度
print("决策树深度：", model.get_depth())  # 输出决策树深度
print("叶节点数目：", model.get_n_leaves())  # 输出叶节点数目
print("每个变量的重要性：", model.feature_importances_)  # 输出每个变量的重要性
...
print('线性回归算法模型的评估得分为：', model.score(X_test, y_test))  # 输出线性回归模型的评估得分
